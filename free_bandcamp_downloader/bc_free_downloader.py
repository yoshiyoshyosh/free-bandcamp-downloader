...
class BCFreeDownloader:
    ...
    def _download_purchased_album(self, user_id: int, tralbum_data: Dict) -> DownloadRet:
    ...
    # all the get_url_x methods are for more resilience against ratelimiting
        ...
        while True:
            try:
                ...
                if e.response.status_code != 429:
                    raise e
                if retries > 6:
                    raise BCFreeDownloadError("Max retries reached.")
                logger.info("Rate limited by bandcamp. Waiting for 15 seconds...")
                time.sleep(15)
                retries += 1
    ...
    def get_page_info(soup: BeautifulSoup) -> PageInfo:
        ...
        if page_type == "band":
            ...
            raise BCFreeDownloadError(f"Page does not have a valid og:type value")
